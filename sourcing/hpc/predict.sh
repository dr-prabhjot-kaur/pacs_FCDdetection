...
#
# Now includes demographic file preparation for meld_graph.
#
# Deployed to:
...
# --- Prepare demographic files for prediction ---------------------------

echo
echo "=========================================="
echo "Preparing demographic files for prediction"
echo "=========================================="

demo_file_list="$study_meld_dir/_demographic_files.txt"
> "$demo_file_list"
...
for subj in "${subjects[@]}"; do
    demo_file="$input_dir/$subj/demographic_features.csv"
    if [ -f "$demo_file" ]; then
        echo "  ✓ Found: $subj/demographic_features.csv"
        echo "$demo_file" >> "$demo_file_list"
        # Show content for verification
        echo "    Content:"
        head -2 "$demo_file" | sed 's/^/      /'
        demo_found=$((demo_found + 1))
        
        # Copy demographic file into bind-mounted /data area so container can access it
        cp -f "$demo_file" "$study_meld_dir/demo_${subj}.csv"
    else
        echo "  ✗ MISSING: $subj/demographic_features.csv"
        demo_missing=$((demo_missing + 1))
    fi
done

echo
echo "Demographic file summary:"
echo "  Found:   $demo_found / ${#subjects[@]}"
echo "  Missing: $demo_missing / ${#subjects[@]}"

if [ $demo_missing -gt 0 ]; then
    echo "  WARNING: Some subjects missing demographic files"
fi
...
echo "  Started: $(date -u +%Y-%m-%dT%H-%M-%SZ)"
echo "  Demographics found: $demo_found / ${#subjects[@]}"
echo "============================================================"
